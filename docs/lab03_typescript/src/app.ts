...
(document.getElementById("searchEmailBtn") as HTMLButtonElement).onclick = () => {
    const email = (document.getElementById("searchEmail") as HTMLInputElement).value; const result = manager.findStudentByEmail(email);
    if (result) {
        showList<Student>([result]); alert(`พบอีเมล: ${result.email}`);
    } else {
